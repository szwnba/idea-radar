...
- **收藏**：信号行和 Top 卡片上的 ★ 按钮，点击存入收藏夹（快照存储，信号滚出滚动窗口也不丢）；顶栏实时计数，收藏页 `/favorites` 统一查看
- **不看**：🚫 按钮立即从雷达和信号流中移除该信号，底部弹出 6 秒撤销提示；隐藏列表可在收藏页恢复（单个恢复或全部恢复）
- 数据保存在浏览器 localStorage，多标签页自动同步，无需账号
